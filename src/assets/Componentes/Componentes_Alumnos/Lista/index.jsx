          ...
          <tbody>
            {alumnos.map((alumno) => (
              <tr key={alumno.id} className="contenido" >
                ...
                <td className="observaciones" onClick={() => showAlert(alumno.observaciones)}>{alumno.observaciones}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
const ListaAlumnos = ({ user }) => {
    ...
    if (confirmDelete) {
      try {
        ...
        if (res.status === 200) {
          ...
          console.error("Error al eliminar alumno: ", res.status, res.statusText);
        }
      } catch (error) {
        console.error("Error al eliminar alumno", error);
      }
  ...
  const updateAlumno = async () => {
    ...
    try {
      console.log("Actualizando alumno con ID:", selectedAlumno.id);
      const res = await axios.put(`${URI}/${selectedAlumno.id}`, selectedAlumno);
      if (res.status === 200) {
        console.log("Alumno actualizado con éxito");
        getAlumnos();
        closeModal();
      } else {
        console.error("Error al actualizar alumno: ", res.status, res.statusText);
      }
    } catch (error) {
      console.error("Error al actualizar alumno", error);
    }
          ...
          <tbody>
            {alumnos.map((alumno) => (
              <tr key={alumno.id} className="contenido">
                ...
                {user === 1 && (
                  <td>
                    <button className="btn btn-danger" onClick={() => deleteAlumno(alumno.id)}>
                      <i className="fas fa-trash-alt"></i>
                    </button>
                    <button className="btn btn-primary" onClick={() => openModal(alumno)}>
                      <i className="fas fa-edit"></i>
                    </button>
                  </td>
                )}
  ...
  {selectedAlumno && (
    <div>
      ...
      <form>
        ...
        <br />
        <div className="botones-modal">
          <button className="btn" type="button" onClick={updateAlumno}>Guardar</button>
